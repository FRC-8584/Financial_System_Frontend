...
function AdminDashboard() {
  ...
  const loadAccountingData = (tab) => {
    ...
    switch (tab) {
      case "income":
        setIncome([{ id: 1, title: "贊助收入", amount: 5000, note: "贊助商A" }]);
        break;
      case "expense":
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-teal-800 text-white p-6 flex flex-col space-y-4">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <button onClick={() => { localStorage.removeItem("token"); navigate("/"); }} className="text-left hover:bg-teal-700 px-3 py-2 rounded">登出</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        {status && (
          <p className={`text-sm mb-4 font-medium ${isSuccess ? "text-green-600 bg-green-50 border border-green-200 p-2 rounded" : "text-red-600 bg-red-50 border-red-200 p-2 rounded"}`}>
            {status}
          </p>
        )}
